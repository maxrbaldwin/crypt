...
const makeShoeEnd = shoe => {
  const insertEndAt = shoe.length - 20;
  shoe.splice(insertEndAt, 0, { end: true });
  return shoe;
};

function makeShoe(decks = 8, intToShuffle = 4) {
  const shoe = makeShoeEnd(multiShuffle(getShoe(decks), intToShuffle));
  shoe.startTotal = shoe.length;
  shoe.getNextCard = () => shoe.shift();
  shoe.getTheCount = (currentCount, nextCount) => currentCount + nextCount;
